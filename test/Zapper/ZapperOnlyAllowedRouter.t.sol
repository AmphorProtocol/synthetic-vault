...
contract VaultZapperOnlyAllowedRouter is OffChainCalls {
    ...
    function test_fail_withdrawAndZapNotAllowedRouter() public {
        vm.startPrank(user);
        vm.expectRevert(
        ...
        zapper.withdrawAndZap(IERC4626(address(0)), address(0), 1 * 1e18, "");
    }
    ...
    function test_fail_redeemAndZapNotAllowedRouter() public {
        vm.startPrank(user);
        vm.expectRevert(
            abi.encodeWithSelector(
                VaultZapper.NotRouter.selector, IERC4626(address(0))
        ...
        zapper.redeemAndZap(IERC4626(address(0)), address(0), 1 * 1e18, "");
    }

    function test_fail_redeemAndZapWithPermitNotAllowedRouter() public {
        vm.startPrank(user);
        vm.expectRevert();
